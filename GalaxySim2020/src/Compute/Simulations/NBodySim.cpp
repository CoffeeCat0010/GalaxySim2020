...
namespace Compute
{
	...
	void NBodySim::run ()
	{
		m_simThread = std::thread ([=]()
		{
				...
				std::cout << localWorkSize << std::endl;
				for ( int i = 0; i < m_numTimesteps && !isShutdown(); i++ )
				{

					p_kern->setKernelArg(0, *p_starPosInBuffer);
					p_kern->setKernelArg(1, *p_starMassBuffer );
					p_kern->setKernelArg(2, softeningFactor);
					p_kern->setKernelArg(3, timeStep);
					p_kern->setKernelArg(4, *p_starVelInBuffer );
					p_kern->setKernelArg(5, *p_starPosOutBuffer);
					p_kern->setKernelArg(6, *p_starVelOutBuffer);
					...
					file->writeTimeStep (Util::clFloatArrToVec3f (result.data(), m_numStars));
					std::vector<cl_float3> resultVel1 = p_starVelOutBuffer->pullFromBuffer(m_numStars);
